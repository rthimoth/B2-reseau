...
# Handler pour écrire dans la console sans couleur (le formattage par défaut)
console_handler = logging.StreamHandler()
console_handler.setFormatter(formatter)
console_handler.addFilter(lambda record: record.levelno == logging.INFO)
logger.addHandler(console_handler)

# Handler et formatter pour les messages de warning avec couleur jaune
warn_handler = logging.StreamHandler()
warn_handler.setFormatter(logging.Formatter('\033[33m%(asctime)s - %(levelname)s - %(message)s\033[0m', datefmt='%Y-%m-%d %H:%M:%S'))
...
try:
    ...
    logger.error(f"Impossible de démarrer le serveur sur {host}:{port} - {e}")
    sys.exit(1)
